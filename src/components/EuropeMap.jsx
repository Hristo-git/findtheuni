...
export default function EuropeMap({ onSelectUni, filters }) {
  ...
  return (
    <div style={{ position: 'relative', background: 'linear-gradient(180deg, #0A0A0B 0%, #111114 100%)', borderRadius: 14, overflow: 'hidden', border: '1px solid rgba(255,255,255,0.08)' }}>
      {/* Legend */}
      <div style={{ position: 'absolute', top: 10, left: 10, background: 'rgba(22,22,24,0.9)', borderRadius: 8, padding: '8px 12px', fontSize: 10, zIndex: 2, backdropFilter: 'blur(4px)' }}>
      ...
      <svg viewBox="0 0 800 600" style={{ width: '100%', height: 400, display: 'block' }}>
        ...
        <path d="M150,180 L200,150 L280,120 L350,100 L420,95 L480,100 L550,110 L600,130 L650,100 L700,120 L720,160 L700,200 L680,240 L700,280 L680,320 L650,350 L600,380 L550,400 L500,420 L450,440 L400,450 L350,440 L300,420 L280,390 L260,350 L240,380 L200,400 L160,380 L140,340 L130,300 L140,260 L160,230 L150,200Z" fill="#1E1E21" stroke="rgba(255,255,255,0.08)" strokeWidth="1" />
        
        {/* Iberian */}
        <path d="M100,350 L140,320 L180,310 L200,340 L240,350 L260,380 L240,420 L200,440 L160,450 L120,430 L90,400 L80,370Z" fill="#1E1E21" stroke="rgba(255,255,255,0.08)" strokeWidth="1" />
        
        {/* Italy */}
        <path d="M380,320 L400,350 L420,380 L430,420 L420,450 L400,460 L390,440 L380,400 L370,370 L360,340Z" fill="#1E1E21" stroke="rgba(255,255,255,0.08)" strokeWidth="1" />
        
        {/* Scandinavia */}
        <path d="M380,30 L400,20 L420,40 L410,80 L400,120 L380,100 L370,60Z" fill="#1E1E21" stroke="rgba(255,255,255,0.08)" strokeWidth="1" />
        <path d="M340,50 L360,30 L370,60 L380,100 L370,130 L350,120 L340,90 L330,70Z" fill="#1E1E21" stroke="rgba(255,255,255,0.08)" strokeWidth="1" />
        
        {/* UK/Ireland */}
        <path d="M200,140 L230,120 L250,140 L240,170 L220,190 L200,180 L190,160Z" fill="#1E1E21" stroke="rgba(255,255,255,0.08)" strokeWidth="1" />
        <path d="M170,150 L190,140 L195,165 L180,180 L165,170Z" fill="#1E1E21" stroke="rgba(255,255,255,0.08)" strokeWidth="1" />
            ...
            <g key={u.id} onClick={() => onSelectUni(u)} onMouseEnter={() => setHover(u.id)} onMouseLeave={() => setHover(null)} style={{ cursor: 'pointer' }}>
              ...
              {isHovered && (
                <g style={{ animation: 'fadeIn .2s ease-out' }}>
                  ...
                  <text x={x} y={y - 28} textAnchor="middle" fontSize={9} fontWeight={600} fill="#FFFFFF" style={{ fontFamily: "'Inter'" }}>{u.emoji} {u.nameEn}</text>
                  <text x={x} y={y - 16} textAnchor="middle" fontSize={8} fill="#A1A1AA" style={{ fontFamily: "'Inter'" }}>#{u.rank} · ⭐{u.rating} · {u.city}</text>
                </g>
              )}
            </g>
          );
