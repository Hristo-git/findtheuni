  ...
  return (
    <div style={{ padding: '32px 0' }} className="page-enter">
      ...
      <div style={{ textAlign: 'center', padding: '40px 0 32px', marginBottom: 20 }}>
        <div style={{ display: 'inline-flex', padding: '4px 14px', background: '#EFF6FF', color: '#2563EB', borderRadius: 14, fontSize: 11, fontWeight: 600, marginBottom: 14 }}>🏫 За университети</div>
        ...
        <p style={{ color: '#78716C', fontSize: 14, maxWidth: 480, margin: '0 auto 20px', lineHeight: 1.6 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(100px,1fr))', gap: 8, marginBottom: 28 }}>
        {[
          [stats.students, 'Студенти', '#2563EB', '#EFF6FF'],
          [stats.profiles, 'Профила', '#7C3AED', '#F5F3FF'],
          [`${stats.countries}`, 'Държави', '#059669', '#ECFDF5'],
          [stats.matchRate, 'Match rate', '#EA580C', '#FFF7ED'],
        ].map(([v, l, cl, bg], i) => (
          <Card key={i} style={{ textAlign: 'center', padding: 12, background: bg, border: 'none', animation: `scaleIn .3s ease-out ${i * 0.06}s both` }}>
            <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 22, fontWeight: 700, color: cl }}>{v}</div>
      ...
      <div style={{ marginBottom: 28 }}>
        <h2 style={{ fontFamily: "'Playfair Display',serif", fontSize: 20, fontWeight: 600, textAlign: 'center', marginBottom: 16 }}>Как работи?</h2>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(200px,1fr))', gap: 10 }}>
          {[
            ...
            <Card key={i} style={{ textAlign: 'center', padding: '18px 14px', animation: `slideIn .3s ease-out ${i * 0.06}s both` }}>
              <div style={{ width: 28, height: 28, borderRadius: '50%', background: '#2563EB', color: 'white', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 700, marginBottom: 6 }}>{s.step}</div>
              <div style={{ fontSize: 24, marginBottom: 4 }}>{s.icon}</div>
              <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 4 }}>{s.title}</div>
              <div style={{ fontSize: 11, color: '#78716C', lineHeight: 1.4 }}>{s.desc}</div>
            </Card>
          ))}
        </div>
      </div>
      ...
      {/* Features */}
      <div style={{ marginBottom: 28 }}>
        <h2 style={{ fontFamily: "'Playfair Display',serif", fontSize: 20, fontWeight: 600, textAlign: 'center', marginBottom: 16 }}>Защо Find The Uni?</h2>
      ...
      <div style={{ marginBottom: 28 }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(260px,1fr))', gap: 10 }}>
          {testimonials.map((t, i) => (
            <Card key={i} style={{ background: '#F5F3FF', border: 'none', padding: '18px' }}>
              <div style={{ fontSize: 12, color: '#57534E', lineHeight: 1.6, fontStyle: 'italic', marginBottom: 10 }}>"{t.quote}"</div>
              <div style={{ fontSize: 11, fontWeight: 600, color: '#7C3AED' }}>{t.author}</div>
      ...
      <div id="b2b-pricing" style={{ marginBottom: 28 }}>
        <h2 style={{ fontFamily: "'Playfair Display',serif", fontSize: 20, fontWeight: 600, textAlign: 'center', marginBottom: 16 }}>Ценообразуване</h2>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(200px,1fr))', gap: 10 }}>
          {pricing.map((p, i) => (
            <Card key={i} style={{ padding: '20px', textAlign: 'center', border: p.accent ? '2px solid #2563EB' : undefined, position: 'relative', animation: `scaleIn .3s ease-out ${i * 0.06}s both` }}>
              {p.accent && <div style={{ position: 'absolute', top: -10, left: '50%', transform: 'translateX(-50%)', padding: '2px 12px', borderRadius: 8, background: '#2563EB', color: 'white', fontSize: 9, fontWeight: 600 }}>Популярен</div>}
              <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 2 }}>{p.name}</div>
              <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 28, fontWeight: 700, color: '#2563EB' }}>{p.price}<span style={{ fontSize: 12, fontWeight: 400, color: '#A8A29E' }}>{p.period}</span></div>
              <div style={{ fontSize: 11, color: '#78716C', marginBottom: 12 }}>{p.desc}</div>
              <div style={{ textAlign: 'left', marginBottom: 14 }}>
                {p.features.map((f, j) => <div key={j} style={{ fontSize: 11, padding: '3px 0', color: '#57534E' }}>✓ {f}</div>)}
              </div>
      ...
      {/* Contact */}
      <div id="b2b-contact">
        <Card style={{ background: 'linear-gradient(135deg,#EFF6FF,#F5F3FF)', border: 'none', padding: '28px', textAlign: 'center' }}>
          <div style={{ fontSize: 32, marginBottom: 8 }}>📩</div>
          <h2 style={{ fontFamily: "'Playfair Display',serif", fontSize: 20, fontWeight: 600, marginBottom: 4 }}>Свържете се с нас</h2>
          <p style={{ fontSize: 12, color: '#78716C', marginBottom: 16, maxWidth: 400, margin: '0 auto 16px' }}>
          ...
          {!sent ? (
            <div style={{ display: 'flex', gap: 8, maxWidth: 360, margin: '0 auto' }}>
              <input value={email} onChange={e => setEmail(e.target.value)} placeholder="university@email.com" type="email"
                style={{ flex: 1, padding: '10px 14px', border: '1px solid #D6D3D1', borderRadius: 10, fontSize: 13, fontFamily: 'inherit', outline: 'none' }} />
              <Btn accent onClick={() => { if (email.includes('@')) setSent(true); }}>Изпрати</Btn>
            </div>
          ) : (
            <div style={{ padding: '16px', background: '#ECFDF5', borderRadius: 12 }}>
              <div style={{ fontSize: 24, marginBottom: 4 }}>✅</div>
              <div style={{ fontSize: 13, fontWeight: 600, color: '#059669' }}>Благодарим!</div>
              <div style={{ fontSize: 12, color: '#065F46' }}>Ще се свържем с вас до 24 часа на {email}</div>
            </div>
          )}
        </Card>
      </div>
