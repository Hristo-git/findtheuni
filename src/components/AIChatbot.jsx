...
function buildAISystem() {
  ...
  const nextAcYear = now.getMonth() >= 7 ? `${currentYear + 1}/${currentYear + 2}` : `${currentYear}/${currentYear + 1}`;
  return `Ти си AI съветник "Find The Uni" — платформа помагаща на български ученици да намерят университет в Европа.
ОТГОВАРЯЙ САМО НА БЪЛГАРСКИ. Бъди конкретен и сбит (max 160 думи). Използвай emoji умерено.
КРИТИЧНО: НИКОГА не използвай символи от азиатски азбуки (китайски, японски, корейски и др.). Използвай САМО кирилица, латиница и стандартни символи.

ТЕКУЩА ДАТА: ${currentDate} | УЧ. ГОДИНА: ${nextAcYear}
...
КЛЮЧОВИ ФАКТИ (${currentYear}):
• Безплатно: Германия (€0+€350/сем), Норвегия (напълно), Финландия (EU/EEA), Чехия/Полша (местен език), Гърция/Австрия (EU €0–1500)
• Виза: EU граждани → без виза в EU. UK → Student visa £490 + IHS £776/год. Швейцария → разрешение СЛЕД пристигане.
• Работа (мин. заплата ${currentYear}): DE 20ч/сед €12.82/ч; UK 20ч/сед £12.21/ч; NL 16ч/сед €13.68/ч; FR 964ч/год €11.88/ч; IE 20ч/сед €13.50/ч; IT без ограничение за EU ~€7–9/ч (no нац. минимум — CCNL договори); Скандинавия/CZ/PL без ограничение; CH 15ч/сед след 6 мес.
• Италия: НЕ е зададена нац. минимална заплата — ставките са по CCNL колективни договори (~€7–9/ч за неквалиф. труд). EU граждани без виза и без ограничение на часовете.
• Erasmus+: грант ВАРИРА по страна/програма (~€350–€600/мес за Италия, не фиксирано €800–1200).
• Стипендии: DAAD €934/мес (САМО за Германия); Chevening пълна (UK); SI SEK 10000/мес (SE); Eiffel €1181/мес (FR); Polimi Merit — намаляване на такси/до €8000/год (не €500–3000).
• Fondazione Cariplo — регионален фонд, условия варират, НЕ е универсална стипендия за Polimi.
• Сертификати: IELTS 6.5+/TOEFL 90+ за англ. (~€220–260); Goethe B2/TestDaF за нем.; DELF B2 за фр.
• Нострификация: ДЗИ е призната автоматично в EU по Болонски процес.
• Срокове ${nextAcYear}: UK-UCAS 15 яну (Oxf/Cam: 15 окт); DE 1–15 юли/дек; NL 1 май(EU)/1 апр; SE 15 яну; NO 15 апр; ETH/EPFL 30 апр; FI 20 яну; IE 1 фев; IT: април–юли (IMAT: авг–септ)
• Разходи: София ~€500; Прага ~€700; Берлин ~€950; Милано ~€900; Париж ~€1200; Лондон ~€1500; Цюрих ~€1650`;
}
const AI_SYSTEM = buildAISystem();
...
// Work hours allowed per country for students (актуализирано 2025)
const countryWorkHours = {
  'UK':          '20 ч/сед. по Student visa (пълно работно ваканция) · мин. £12.21/ч',
  'Германия':    '20 ч/сед. (120 пълни дни или 240 полудни/год.) · мин. €12.82/ч',
  'Австрия':     '20 ч/сед. · мин. ~€10.50/ч',
  'Швейцария':   '15 ч/сед. след 6 мес. пребиваване · мин. CHF 20–23/ч',
  'Франция':     '964 ч/год. (~18.7 ч/сед.) · SMIC €11.88/ч',
  'Нидерландия': '16 ч/сед. (или пълно юни–август) · мин. €13.68/ч',
  'Белгия':      '475 ч/год. · мин. ~€11.90/ч',
  'Финландия':   '25 ч/сед. · мин. ~€9–13/ч (по сектор/CCNL)',
  'Ирландия':    '20 ч/сед. (40 ваканция) · мин. €13.50/ч',
  'Испания':     '30 ч/сед. за EU студенти · мин. €8.62/ч',
  'Италия':      'EU граждани — БЕЗ ограничение на часовете · ~€7–9/ч (по CCNL договор, без нац. минимум)',
  'Португалия':  '40 ч/сед. за EU · мин. €8.58/ч',
};
